...
const float SENSOR_BACK_STEP = 3300; // 8 degrees 2850
...
void moveMotor(int angle, int speedPercent) {
  ...
  int step_target_position = (angle / STEP_ANGLE) + _step_zero_angle;
  ...
  while (_step_current_position != step_target_position) {
    ...
    if (_step_current_position >= _step_end_position) {
      Serial.print("ST,0,RETMOVE,ERR_LESS," + String(retAngle) + "," + String(speedPercent) +
                   ",ED\r\n");
      return;
    }

    if (_step_current_position <= _step_start_position) {
      Serial.print("ST,0,RETMOVE,ERR_OVER," + String(retAngle) + "," + String(speedPercent) +
                   ",ED\r\n");
      return;
    }
  }
